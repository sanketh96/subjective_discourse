...
class CongressionalHearingLogitsSortedProcessor(BertProcessor):
    NUM_CLASSES = 6
    IS_MULTILABEL = True
    ...
    def __init__(self, config=None):
        ...
        if config and config.fold_num >= 0:
            self.NAME = os.path.join('CongressionalHearingLogitsSortedFolds', 'fold'+str(config.fold_num))
        else:
            self.NAME = 'CongressionalHearingLogitsSorted'
