...
from transformers import BertModel, RobertaModel, XLNetModel, DebertaModel
...
class DebertaHierarchical(nn.Module):

    def __init__(self, model_name, num_fine_labels, num_coarse_labels, use_second_input=False):
        super().__init__()

        self.deberta = DebertaModel.from_pretrained(model_name, num_labels=num_fine_labels)
        self.dropout = nn.Dropout(self.deberta.config.hidden_dropout_prob)
        self.classifier = DebertaClassificationHeads(self.deberta.config, num_coarse_labels, num_fine_labels)

        # hacky fix for error in transformers code
        # that triggers error "Assertion srcIndex < srcSelectDimSize failed"
        # https://github.com/huggingface/transformers/issues/1538#issuecomment-570260748
        # if use_second_input:
        #     self.deberta.config.type_vocab_size = 2
        #     single_emb = self.deberta.embeddings.token_type_embeddings
        #     self.deberta.embeddings.token_type_embeddings = torch.nn.Embedding(2, single_emb.embedding_dim)
        #     self.deberta.embeddings.token_type_embeddings.weight = torch.nn.Parameter(single_emb.weight.repeat([2, 1]))
...
class DebertaClassificationHeads(nn.Module):
    """Head for sentence-level classification tasks."""
